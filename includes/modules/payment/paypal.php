...
class paypal {
	...
	function install() {
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Adresse eMail', 'MODULE_PAYMENT_PAYPAL_ID', '', 'Ins�rez l\'adresse mail du compte Paypal recevant les paiements', '6', '4', now())");
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Devise des transactions', 'MODULE_PAYMENT_PAYPAL_CURRENCY', 'Choisissez une devise', 'La devise utilis�e pour les paiements par CB', '6', '6', 'tep_cfg_select_option(array(\'Selected Currency\',\'Only USD\',\'Only CAD\',\'Only EUR\',\'Only GBP\',\'Only JPY\'), ', now())");
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Serveur de paiement', 'MODULE_PAYMENT_PAYPAL_TRANSACTION_SERVER', 'Live', 'Le serveur de paiement � utiliser : Live(production), Sandbox(test)', '6', '3', 'tep_cfg_select_option(array(\'Live\', \'Sandbox\'), ', now())");
		...
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Zone de paiement', 'MODULE_PAYMENT_PAYPAL_ZONE', '0', 'Si une zone est s�lectionn�e, seule cette zone proposera ce type de paiement.', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
		tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Statut des commandes', 'MODULE_PAYMENT_PAYPAL_ORDER_STATUS_ID', '0', 'D�finissez le statut qui sera assign� aux commandes pay�es avec ce mode de paiement.', '6', '0', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
	}
